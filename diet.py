...
# 1. Define a method that returns the string "GET BACK ON YOUR DIET!" if Cookie Monster has eaten any cookies.
...
# 3. The longer Cookie Monster is on this diet, the more we feel we can trust him. Let's write a flex_day method.
#    Let's write a method that takes in two arguments - a goal number of cookies and current number of cookies.
#    It will do the same as the previous method did, so calling flex_day(2, 3) should return a scolding string, since he ate one more than his goal.



# 4. CHALLENGE: Dieting isn't all about cutting out unhealthy foods; it's also about balance.
#    Let's define a method called balanced_diet that takes in two arguments - a number of cookies and a number of vegetables.
